...
void CShadeButtonST::SetShade(UINT shadeID, BYTE granularity, BYTE highlight, BYTE coloring, COLORREF color)
{
	...
	m_dh.Create(max((long)1,sXSize /*-2*m_FocusRectMargin-1*/ ),1,8);	//create the horizontal focus bitmap
	m_dv.Create(1,max((long)1,sYSize /*-2*m_FocusRectMargin*/),8);	//create the vertical focus bitmap
...
//----------------------------------------------------
	case 7:	// SHS_HARDBUMP
		...
		//set vertical bump
		d=min((long)16,sXSize/6);	//max edge=16
		a=sYSize*sYSize/4;
		posDst =iDst;
		for(i = 0; i < sYSize; i++) {
			y=i-sYSize/2;
