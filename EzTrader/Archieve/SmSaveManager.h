...
namespace DarkHorse {
	...
	public:
		...
		void save_total_asset_windows2(const std::string& filename, const std::map<HWND, std::shared_ptr<VtAccountAssetDlg>>& map_to_save);
		void restore_total_asset_windows_from_json2(CWnd* parent_window, const std::string& filename, std::map<HWND, std::shared_ptr<VtAccountAssetDlg>>& map_to_restore);
