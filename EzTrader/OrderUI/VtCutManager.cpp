...
void VtCutManager::AddOrderHd(std::shared_ptr<HdOrderRequest> req)
{
	if (!req)
		return;
...
{
	CString msg;
	//msg.Format("AddStopOrderForFilled1:: order->no = %d, order_state = %d, order_remain = %d, order_type = %d\n", order->orderNo, (int)order->state, order->remainQty, order->Type);
//	TRACE(msg);
	if (!_OrderConfigMgr || !order || (!_EnableCutProfit && !_EnableCutLoss))
		return;

	//msg.Format("AddStopOrderForFilled2:: order->no = %d, order_state = %d, order_remain = %d, order_type = %d\n", order->orderNo, (int)order->state, order->remainQty, order->Type);
	//TRACE(msg);
	// 모드를 확인한다.
	int mode = _EnableCutProfit && _EnableCutLoss ? 2 : (_EnableCutProfit ? 0 : 1);

	// 외부 주문인 경우 아직 주문 타입이 -1이다. 
	// 그러나 주문 타입이 정해지지 않는 경우는 접수가 먼저 오고 서버 주문 도착이 오는 경우는 주문 타입은 여전히 -1이다. 
	// 이때는 주문 타입에 상관없이 그냥 손절과 익절을 설정해야 한다.
	// 물론 외부 주문도 손절과 익절이 진행이 된다. 즉 모든 주문에 대하여 손절과 익절 설정이 진행이 된다.
	/*
...
std::shared_ptr<HdOrderRequest> VtCutManager::FindStop(int reqId)
{
	auto it = StopOrderMapHd.find(reqId);
	if (it != StopOrderMapHd.end())
		return it->second;
...
void VtCutManager::RecalPosByAvg(VtSymbol* sym)
{
	...
	for (auto it = StopOrderMapHd.begin(); it != StopOrderMapHd.end(); ++it) {
		...
		// 요청 포지션에 따라 위치를 찾아 설정해 준다.
		// 요청 포지션은 현재 포지션과 반대이다.
		// 따라서 위치도 반대로 생각해 줘야 한다.
...
void VtCutManager::PutOrder(VtOrder* order, VtSymbol* sym, int reqType, VtPriceType priceType)
{
	...
	if (order->Type == 0) { // 실계좌 주문
		...
		acnt = mainApp.AcntMgr().SubAcntMgr().FindAccount(order->SubAccountNo);
	}
