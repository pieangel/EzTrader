...
LRESULT SmOrderGrid::OnQuoteChangedMessage(WPARAM wParam, LPARAM lParam)
{
	...
	// 포지션이 있다면 포지션 표시를 다시 그린다.
	ClearPositionInfo(refreshSet);
	SetPositionInfo(refreshSet);

	// 스탑 주문이 나갔을 수도 있기 때문데 스탑 주문을 다시 그려준다.
	ClearOldStopOrders(refreshSet);
	SetStopOrderInfo(refreshSet);
	CalcPosStopOrders(refreshSet);
...
// 
	...
	CString msg;
	msg.Format("OnHogaChangedMessage %s, %d\n", symbol->ShortCode.c_str(), _HogaCount++);
	//TRACE(msg);
	}
	catch (std::exception& e) {
		LOG_F(ERROR, _T(" %s, MSG : %s"), __FUNCTION__, e.what());
...
void SmOrderGrid::OnOrderEvent(VtOrder* order)
{
	...
	if (!_CenterWnd || !_CutMgr || !_CenterWnd->Symbol()) {
		...
		if (order->state == VtOrderState::Filled) {
			...
			// 내일은 order->RemainQty를 조사하여 왜 추가가 안되는지 알아본다.
			//test.Format("event:: order->no = %d, order_filled_count = %d, order_state = %d, order_remain = %d\n", order->orderNo, filled_count++, (int)order->state, order->remainQty);
			//TRACE(test);
			//refreshSet.clear();
			// 스탑 주문 추가
			_CutMgr->AddStopOrderForFilled(_CenterWnd->Symbol(), order);
			...
			// 스탑 주문 표시
			SetStopOrderInfo(refreshSet);
			// 스탑 주문 표시 범위 계산 - 스탑 주문은 표시 범위가 넓다.
			CalcPosStopOrders(refreshSet);
			// 셀 갱신 
			RefreshCells(refreshSet);
		}
	...
	for (int i = 0; i < 5; i++) {
		...
		if (i == 0) {
			...
			for (int k = 0; k < m_nCols; ++k) {
				...
				if (pCell) {
					pCell->Style(1);
					refreshSet.insert(std::make_pair(pos, k));
				}
			}
		}
		if (pos >= _StartRowForValue && pos <= _EndRowForValue) {
...
void SmOrderGrid::CalcPosStopOrders(std::set<std::pair<int, int>>& refreshSet)
{
	...
	for (auto it = _StopOrderMgr->StopOrderMapHd.begin(); it != _StopOrderMgr->StopOrderMapHd.end(); ++it) {
		const std::shared_ptr<HdOrderRequest> order = it->second;
	...
	const std::map<int, std::shared_ptr<HdOrderRequest>>& profitLossMap = _CutMgr->GetStopOrderMap();
	for (auto it = profitLossMap.begin(); it != profitLossMap.end(); ++it) {
		const std::shared_ptr<HdOrderRequest> order = it->second;
		int order_row = FindRowFromCenterValue(order->Price);
		CCellID order_cell, slip_cell;
...
{
	auto first_pos = refreshSet.begin();
	if (first_pos == refreshSet.end())
		return;
	...
	min_row = first.first;
	max_row = first.first;
	min_col = first.second;
	max_col = first.second;
	for (auto it = refreshSet.begin(); it != refreshSet.end(); ++it) {
		std::pair<int, int> pos = *it;
		//CGridCellBase* pCell = GetCell(pos.first, pos.second);
		//InvalidateCellRect(pos.first, pos.second);
		if (pos.first < min_row)
			min_row = pos.first;
		if (pos.first > max_row)
			max_row = pos.first;
		if (pos.second < min_col)
			min_col = pos.second;
		if (pos.second > max_col)
			max_col = pos.second;
	}

	CCellRange range(min_row, min_col, max_row, max_col);
...
void SmOrderGrid::PutOrder(VtPosition* posi, int price, bool liqud /*= false*/)
{
	...
	request->orderType = VtOrderType::New;
	// 정상 주문인지 청산주문인지 구별
	// 0 : 일반 , 1 : 익절 청산, 2 : 손절 청산- 청산 주문일 때는 익절 손절이 활성화 되어 있어도 처리하지 않는다.
...
void SmOrderGrid::OnTimer(UINT_PTR nIDEvent)
{
	RefreshAllValues();
}
