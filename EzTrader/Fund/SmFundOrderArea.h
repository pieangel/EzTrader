...
private:
	bool _SettingCenterValue = false;
	void DrawStopOrder();
	...
	void PutFilledOrder(const DarkHorse::SmPositionType& type, const int& price, const DarkHorse::SmPriceType& price_type);
	...
	// Key : account no, Value : account password
	//std::map<std::string, std::string> _AccountPwdMap;
