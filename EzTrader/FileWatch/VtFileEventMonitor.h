...
private:
	...
	{
		switch (dwAction) {
		case FILE_ACTION_ADDED: // 파일이 추가됨
			return _T("[Add]");
		case FILE_ACTION_REMOVED: // 파일이 삭제됨
			return _T("[Delete]");
		case FILE_ACTION_MODIFIED: // 파일이 수정됨
			return _T("[Modified]");
		case FILE_ACTION_RENAMED_OLD_NAME: // 파일이름이 예전이름에서 변경됨
			return _T("[Renamed From]");
		case FILE_ACTION_RENAMED_NEW_NAME:
			return _T("[Renamed To]"); // 파일 이름이 새로 변경됨
		default:
			return _T("[BAD DATA]"); // 잘못된 데이터
		}
	}
