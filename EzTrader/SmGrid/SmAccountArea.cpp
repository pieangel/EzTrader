...
BEGIN_MESSAGE_MAP(SmAccountArea, CBCGPStatic)
	//{{AFX_MSG_MAP(CBCGPTextPreviewCtrl)
	ON_WM_PAINT()
	ON_WM_TIMER()
END_MESSAGE_MAP()

SmAccountArea::SmAccountArea()
{
...
void SmAccountArea::SetUp()
{
	...
	mainApp.CallbackMgr()->SubscribeQuoteCallback((long)this, std::bind(&SmAccountArea::OnQuoteEvent, this, _1));
	mainApp.CallbackMgr()->SubscribeOrderCallback((long)this, std::bind(&SmAccountArea::OnOrderEvent, this, _1, _2));
...
void SmAccountArea::UpdateAssetInfo()
{
	_Mode == 0 ? UpdateAccountAssetInfo() : UpdateFundAssetInfo();
}

void SmAccountArea::OnQuoteEvent(const std::string& symbol_code)
{
	_EnableQuoteShow = true;
}

void SmAccountArea::OnOrderEvent(const std::string& account_no, const std::string& symbol_code)
{
	_EnableOrderShow = true;
}

void SmAccountArea::UpdateAccountAssetInfo()
{
	if (!_Account) return;
...
void SmAccountArea::UpdateFundAssetInfo()
{
	if (!_Fund) return;
...
void SmAccountArea::CreateResource()
{
...
void SmAccountArea::OnTimer(UINT_PTR nIDEvent)
{
	bool needDraw = false;
	if (_EnableQuoteShow) {
		UpdateAssetInfo();
