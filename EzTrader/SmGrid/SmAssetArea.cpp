...
BEGIN_MESSAGE_MAP(SmAssetArea, CBCGPStatic)
	//{{AFX_MSG_MAP(CBCGPTextPreviewCtrl)
	ON_WM_PAINT()
	ON_WM_TIMER()
END_MESSAGE_MAP()

SmAssetArea::SmAssetArea()
{
...
void SmAssetArea::SetUp()
{
	...
	mainApp.CallbackMgr()->SubscribeQuoteCallback((long)this, std::bind(&SmAssetArea::OnQuoteEvent, this, _1));
	mainApp.CallbackMgr()->SubscribeOrderCallback((long)this, std::bind(&SmAssetArea::OnOrderEvent, this, _1, _2));

	SetTimer(1, 40, NULL);
}

void SmAssetArea::OnPaint()
{
...
void SmAssetArea::UpdateSymbolInfo()
{
	if (!_Symbol) return;
...
void SmAssetArea::SetAssetInfo()
{
	_Mode == 0 ? SetAccountAssetInfo() : SetFundAssetInfo();
}

void SmAssetArea::OnQuoteEvent(const std::string& symbol_code)
{
	_EnableQuoteShow = true;
}

void SmAssetArea::OnOrderEvent(const std::string& account_no, const std::string& symbol_code)
{
	_EnableOrderShow = true;
}

void SmAssetArea::SetAccountAssetInfo()
{
	if (!_Account) return;
...
void SmAssetArea::SetFundAssetInfo()
{
	if (!_Fund) return;
...
void SmAssetArea::CreateResource()
{
...
void SmAssetArea::OnTimer(UINT_PTR nIDEvent)
{
	bool needDraw = false;
	if (_EnableQuoteShow) {
		SetAssetInfo();
