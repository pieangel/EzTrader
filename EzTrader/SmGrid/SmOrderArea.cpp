...
BEGIN_MESSAGE_MAP(SmOrderArea, CBCGPStatic)
	...
	ON_WM_CREATE()
	ON_WM_LBUTTONDBLCLK()
	ON_MESSAGE(WM_SYMBOL_MASTER_RECEIVED, &SmOrderArea::OnWmSymbolMasterReceived)
	ON_MESSAGE(WM_QUOTE_RECEIVED, &SmOrderArea::OnWmQuoteReceived)
	ON_MESSAGE(WM_HOGA_RECEIVED, &SmOrderArea::OnWmHogaReceived)
	ON_MESSAGE(WM_ORDER_CHANGED, &SmOrderArea::OnUmOrderChanged)
	ON_WM_TIMER()
END_MESSAGE_MAP()

BOOL SmOrderArea::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
void SmOrderArea::SetQuote(std::shared_ptr<DarkHorse::SmSymbol> symbol)
{
	if (!symbol) return;
...
void SmOrderArea::SetHoga(std::shared_ptr<DarkHorse::SmSymbol> symbol)
{
...
void SmOrderArea::ClearOldHoga(DarkHorse::Hoga_Type hoga_type) const noexcept
{
	std::shared_ptr<SmCell> pCell = nullptr;
...
void SmOrderArea::ClearOldQuote()
{
	std::shared_ptr<SmCell> pCell = nullptr;
...
void SmOrderArea::ClearStopOrders()
{
	std::shared_ptr<SmCell> pCell = nullptr;
...
int SmOrderArea::FindRow(const int& value) const noexcept
{
	int result = -1;
...
int SmOrderArea::FindRowFromCenterValue(std::shared_ptr<DarkHorse::SmSymbol> symbol, const int& value)
{
...
void SmOrderArea::SetQuoteColor(std::shared_ptr<DarkHorse::SmSymbol> symbol)
{
...
int SmOrderArea::FindValue(const int& row) const noexcept
{
	int result = -1;
...
void SmOrderArea::DrawHogaLine(const CRect& rect)
{
	if (!_Symbol) return;
...
void SmOrderArea::DrawFixedSelectedCell()
{
...
void SmOrderArea::DrawMovingOrder()
{
	if (_DraggingOrder) {
...
void SmOrderArea::SetUp()
{
	...
	mainApp.CallbackMgr()->SubscribeQuoteCallback((long)this, std::bind(&SmOrderArea::OnQuoteEvent, this, _1));
	mainApp.CallbackMgr()->SubscribeHogaCallback((long)this, std::bind(&SmOrderArea::OnHogaEvent, this, _1));
...
int SmOrderArea::RecalRowCount(const int& height)
{
	CRect rect;
...
void SmOrderArea::UpdateOrder(const std::string& symbol_code)
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::ConnectOrderUpdateSignal()
{
	if (!_Account || !_Symbol) return;

	std::shared_ptr<SmSymbolOrderManager> symbol_order_mgr = mainApp.TotalOrderMgr()->FindAddSymbolOrderManager(_Account->No(), _Symbol->SymbolCode());
	symbol_order_mgr->order_update.Connect(this, &SmOrderArea::UpdateOrder);
}

void SmOrderArea::ArrangeCenterValue()
{
	ClearOldQuote();
	ClearOldHoga();
	ClearOrders();
...
void SmOrderArea::BuyByMarketPrice()
{
	PutOrder(SmPositionType::Buy, 0, SmPriceType::Market);
}

void SmOrderArea::SellByMarketPrice()
{
	PutOrder(SmPositionType::Sell, 0, SmPriceType::Market);
}

void SmOrderArea::CancelSellStop()
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::CancelSellOrder()
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::CancelAllOrder()
{
	CancelBuyStop();
	CancelSellStop();
	CancelSellOrder();
...
void SmOrderArea::CancelBuyStop()
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::CancelBuyOrder()
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::ProcesButtonClick(const std::shared_ptr<SmCell>& cell)
{
	if (!cell) return;
...
bool SmOrderArea::ProcesButtonClickByPos(const CPoint& point)
{
...
std::pair<int, int> SmOrderArea::GetOrderCount(const std::shared_ptr<SmCell>& cell)
{
...
void SmOrderArea::PutOrderBySpaceBar()
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::ChangeOrderByKey(const int up_down)
{
	if (!_Account || !_Symbol) return;
	Invalidate();
}
...
void SmOrderArea::OnOrderEvent(const std::string& account_no, const std::string& symbol_code)
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::OnQuoteEvent(const std::string& symbol_code)
{
	if (!_Symbol) return;
	if (_Symbol->SymbolCode() != symbol_code) return;
	if (_FixedMode) {
...
void SmOrderArea::OnHogaEvent(const std::string& symbol_code)
{
	if (!_Symbol) return;
	if (_Symbol->SymbolCode() != symbol_code) return;
	_EnableHogaShow = true;
...
void SmOrderArea::OnSymbolMasterEvent(const std::string& symbol_code)
{
	if (!_Symbol) return;
	if (_Symbol->SymbolCode() != symbol_code) return;
}

void SmOrderArea::UpdateOrderSettings(SmOrderSettings settings)
{
	_OrderSettings = settings;
}

void SmOrderArea::SetAllRowHeight(const int& row_height)
{
	_Grid->SetAllRowHeight(row_height);
}

void SmOrderArea::FixedMode(bool val)
{
	_FixedMode = val;
	ArrangeCenterValue();
	Refresh();
...
void SmOrderArea::Symbol(std::shared_ptr<DarkHorse::SmSymbol> val)
{
	_Symbol = val;
	ArrangeCenterValue();
	Invalidate();
...
void SmOrderArea::DrawStopOrder()
{
...
void SmOrderArea::DrawArrow(const CBCGPPoint& start_point, const CBCGPPoint& end_point, const double& stroke_width, const int& head_width)
{
...
void SmOrderArea::ProcessFixedMode()
{
...
void SmOrderArea::SetProfitLossCut(std::shared_ptr<SmOrderRequest> order_req)
{
...
void SmOrderArea::SetStopOrderCut(std::shared_ptr<SmOrderRequest> order_req)
{
	order_req->CutSlip = _OrderSettings.SlipTick;
}

void SmOrderArea::PutStopOrder(const DarkHorse::SmPositionType& type, const int& price)
{
	if (!_Account || !_Symbol) return;
	const auto symbol_order_mgr = mainApp.TotalOrderMgr()->FindAddSymbolOrderManager(_Account->No(), _Symbol->SymbolCode());
...
void SmOrderArea::PutOrder(const SmPositionType& type, const int& price, const SmPriceType& price_type)
{
	if (!_Account || !_Symbol) return;
...
void SmOrderArea::CreateResource()
{
...
void SmOrderArea::IncreaseCloseRow(const int& delta)
{
	_CloseRow += delta;
	if (!_Symbol) return;
...
void SmOrderArea::DrawMovingRect()
{
...
/*
void SmOrderArea::CancelOrder(const SmPositionType& position, const int& src_price)
{
	if (!_Account || !_Symbol) return;

	std::map< std::string, std::shared_ptr<SmOrder>> order_vec;

	

	std::shared_ptr<SmSymbolOrderManager> symbol_order_mgr = mainApp.TotalOrderMgr()->FindAddSymbolOrderManager(_Account->No(), _Symbol->SymbolCode());

	if (position == SmPositionType::Buy) symbol_order_mgr->BuyOrderViewer()->GetOrderVec(src_price, order_vec);
	else if (position == SmPositionType::Sell) symbol_order_mgr->SellOrderViewer()->GetOrderVec(src_price, order_vec);

	for (auto it = order_vec.begin(); it != order_vec.end(); ++it) {
		auto order_req = SmOrderRequestManager::MakeCancelOrderRequest(
			_Account->No(),
			_Account->Pwd(), _Symbol->SymbolCode(),
			it->second->OrderNo, src_price, position,
...
void SmOrderArea::CancelOrder(const std::shared_ptr<DarkHorse::SmCell>& src_cell)
{
	if (!src_cell) return;
...
/*
void SmOrderArea::ChangeOrder(const SmPositionType& position, const int& src_price, const int& tgt_price)
{
	if (!_Account || !_Symbol) return;
	...
	for (auto it = order_vec.begin(); it != order_vec.end(); ++it) {
		auto order_req = SmOrderRequestManager::MakeChangeOrderRequest(
			_Account->No(), 
			_Account->Pwd(), _Symbol->SymbolCode(), 
			it->second->OrderNo, tgt_price, position, 
			it->second->OrderAmount);
...
void SmOrderArea::ChangeOrder(const std::shared_ptr<DarkHorse::SmCell>& src_cell, const int& tgt_price)
{
	if (!src_cell) return;
	if (src_cell->GetOrderCount() == 0) return;
	const std::map<int, std::shared_ptr<SmOrder>>& order_map = src_cell->GetOrderMap();
...
void SmOrderArea::ChangeStop(const std::shared_ptr<DarkHorse::SmCell>& src_cell, const std::shared_ptr<DarkHorse::SmCell>& tgt_cell, const int& src_price, const int& tgt_price)
{
	if (!src_cell) return;
...
void SmOrderArea::CancelStop(const std::shared_ptr<DarkHorse::SmCell>& src_cell)
{
	if (!src_cell) return;
...
void SmOrderArea::ProcessButtonMsg(const BUTTON_ID& id)
{
	switch (id)
	{
...
void SmOrderArea::OnMouseMove(UINT nFlags, CPoint point)
{
	CString msg;
...
void SmOrderArea::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CString msg;
...
void SmOrderArea::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	CString msg;
...
void SmOrderArea::OnMouseHover(UINT nFlags, CPoint point)
{
	_Hover = true;
	Invalidate();
}

void SmOrderArea::OnMouseLeave()
{
	if (_MouseMode == MouseMode::Normal) {
		ClearOldSelectedCells();
...
void SmOrderArea::OnLButtonDown(UINT nFlags, CPoint point)
{
	SetFocus();
	...
	if (_MainDialog ) {
		...
		AbAccountOrderCenterWindow* wnd = (AbAccountOrderCenterWindow*)GetParent();
...
void SmOrderArea::OnRButtonDown(UINT nFlags, CPoint point)
{
	if (_MouseMode == MouseMode::Normal) {
		///ClearOldSelectedCells();
...
void SmOrderArea::OnLButtonUp(UINT nFlags, CPoint point)
{
...
int SmOrderArea::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void SmOrderArea::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (!_Grid) return;
...
LRESULT SmOrderArea::OnWmSymbolMasterReceived(WPARAM wParam, LPARAM lParam)
{
...
LRESULT SmOrderArea::OnWmQuoteReceived(WPARAM wParam, LPARAM lParam)
{
	const int symbol_id = static_cast<int>(wParam);
	if (!_Symbol || _Symbol->Id() != symbol_id) return 0;
...
LRESULT SmOrderArea::OnWmHogaReceived(WPARAM wParam, LPARAM lParam)
{
	const int symbol_id = static_cast<int>(wParam);
	if (!_Symbol || _Symbol->Id() != symbol_id) return 0;
...
LRESULT SmOrderArea::OnUmOrderChanged(WPARAM wParam, LPARAM lParam)
{
	UpdateOrder(_Symbol->SymbolCode());
	Invalidate(FALSE);
	return 1;
...
void SmOrderArea::OnTimer(UINT_PTR nIDEvent)
{
	bool needDraw = false;
